...
try {
    ...
    if ($action == 'update_permissions') {
        ...
        $permissionIdMap = [];
        $permissionRows = $db->query("SELECT id, code FROM permissions")->fetchAll(PDO::FETCH_ASSOC);
        ...
        try {
            ...
            if ($hasLegacyRolePermissionsTable) {
                $deleteLegacyStmt = $db->prepare("DELETE FROM role_permissions WHERE role_id = ?");
                $deleteLegacyStmt->execute([$roleId]);

                if (!empty($selectedPermissionIds)) {
                    $insertLegacyStmt = $db->prepare("INSERT INTO role_permissions (role_id, permission_id) VALUES (?, ?)");
