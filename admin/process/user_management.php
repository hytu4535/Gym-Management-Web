...
if ($action == 'add') {
    ...
    if ($hasPhoneColumn) {
        $sql = "INSERT INTO users (username, password, email, phone, role_id, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, NOW())";
        $stmt = $db->prepare($sql);
        $stmt->execute([$username, $password, $email, $phone, $role_id, 'active']);
    } else {
        $sql = "INSERT INTO users (username, password, email, role_id, status, created_at)
            VALUES (?, ?, ?, ?, ?, NOW())";
        $stmt = $db->prepare($sql);
        $stmt->execute([$username, $password, $email, $role_id, 'active']);
    }
...
if ($action == 'edit') {
    ...
    $currentUser = null;
    if (empty($errors)) {
        $currentSql = "SELECT username, email FROM users WHERE id = ? LIMIT 1";
    ...
    if ($hasPhoneColumn) {
        if (!empty($password)) {
            $sql = "UPDATE users SET username=?, email=?, phone=?, role_id=?, password=? WHERE id=?";
            $stmt = $db->prepare($sql);
            $stmt->execute([$currentUser['username'], $currentUser['email'], $phone, $role_id, $password, $id]);
        } else {
            $sql = "UPDATE users SET username=?, email=?, phone=?, role_id=? WHERE id=?";
            $stmt = $db->prepare($sql);
            $stmt->execute([$currentUser['username'], $currentUser['email'], $phone, $role_id, $id]);
        }
    } else {
        if (!empty($password)) {
            $sql = "UPDATE users SET username=?, email=?, role_id=?, password=? WHERE id=?";
            $stmt = $db->prepare($sql);
            $stmt->execute([$currentUser['username'], $currentUser['email'], $role_id, $password, $id]);
        } else {
            $sql = "UPDATE users SET username=?, email=?, role_id=? WHERE id=?";
            $stmt = $db->prepare($sql);
            $stmt->execute([$currentUser['username'], $currentUser['email'], $role_id, $id]);
        }
    }
