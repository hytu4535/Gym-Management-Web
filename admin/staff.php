...
$stmt = $db->prepare(
  "SELECT s.id, s.user_id AS users_id, s.full_name, s.position, $departmentSelectSql, s.status, u.username, u.email, u.phone, u.role_id AS linked_role_id
   FROM staff s
   LEFT JOIN users u ON s.user_id = u.id" . $departmentJoinSql . $whereSql . " ORDER BY s.id DESC"
);
$stmt->execute($whereParams);
$staffList = $stmt->fetchAll();
$usedUserIds = array_map('intval', $db->query("SELECT user_id FROM staff")->fetchAll(PDO::FETCH_COLUMN));
