...
$sql_member = "SELECT full_name, phone, height, weight FROM members WHERE users_id = ?";
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="height" value="<?php echo (!empty($user['weight']) && (float)$user['weight'] > 0) ? (float)$user['weight'] : ''; ?>" placeholder="0" class="form-control">
                                    <small class="text-danger error-text mt-1" style="display:none; font-weight: 500;"></small>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="weight" value="<?php echo (!empty($user['weight']) && (float)$user['weight'] > 0) ? (float)$user['weight'] : ''; ?>" placeholder="0" class="form-control">
                                    <small class="text-danger error-text mt-1" style="display:none; font-weight: 500;"></small>
                                </div>
                            </div>
                        </div>
...
function showErrorInline(inputName, message, formId) {
    const form = document.getElementById(formId);
    const input = form.querySelector('[name="' + inputName + '"]');
    if (!input) return;

    input.classList.add('is-invalid');
    const errorSmall = input.parentElement.querySelector('.error-text');
    if (errorSmall) {
...
function clearErrors(formId) {
    ...
    const errorSmalls = form.querySelectorAll('.error-text');
    ...
    if (avatarInput) {
        avatarInput.addEventListener('change', function() {
            ...
            if (file.size > 5 * 1024 * 1024) {
                showErrorInline('avatar', 'Kích thước ảnh không được vượt quá 5MB!', 'profile-form');
                this.value = '';
                return;
            }
...
document.getElementById('profile-form').addEventListener('submit', function(e) {
    ...
    if (heightInput.value.trim() !== '') {
        let h = Number(heightInput.value.trim());
        if (isNaN(h) || h <= 0 || h > 300) {
            return showErrorInline('height', 'Chiều cao phải là số hợp lệ (> 0)!', 'profile-form');
        }
    }
    if (weightInput.value.trim() !== '') {
        let w = Number(weightInput.value.trim());
        if (isNaN(w) || w <= 0 || w > 300) {
            return showErrorInline('weight', 'Cân nặng phải là số hợp lệ (> 0)!', 'profile-form');
        }
    }
    ...
    .then(data => {
        if (!data.success && data.fieldErrors) {
            Object.keys(data.fieldErrors).forEach(key => {
                showErrorInline(key, data.fieldErrors[key], 'profile-form');
            });
        } else {
...
document.getElementById('change-password-form').addEventListener('submit', function(e) {
    ...
    const currentPass = document.querySelector('#change-password-form [name="current_password"]');
    const newPass = document.querySelector('#change-password-form [name="new_password"]');
    const confirmPass = document.querySelector('#change-password-form [name="confirm_password"]');

    if (currentPass.value === '') {
        return showErrorInline('current_password', 'Vui lòng nhập mật khẩu hiện tại!', 'change-password-form');
    }
    if (newPass.value === '') {
        return showErrorInline('new_password', 'Vui lòng nhập mật khẩu mới!', 'change-password-form');
    }
    if (newPass.value.length < 6) {
        return showErrorInline('new_password', 'Mật khẩu mới phải có ít nhất 6 ký tự!', 'change-password-form');
    }
    if (newPass.value !== confirmPass.value) {
        return showErrorInline('confirm_password', 'Mật khẩu xác nhận không khớp!', 'change-password-form');
    }
    ...
    .then(data => {
        if (!data.success && data.fieldErrors) {
            Object.keys(data.fieldErrors).forEach(key => {
                showErrorInline(key, data.fieldErrors[key], 'change-password-form');
            });
        } else {
            showMessage(data.message, data.success ? 'success' : 'error');
            if (data.success) {
